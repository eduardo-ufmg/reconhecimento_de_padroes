...
def plot_k_for_set(setname, X, y):
  ...
  k_values = list(range(num_instances//100, num_instances//10, num_instances//100))
  ...
  plot_accuracyk(k_values, average_accuracies, setname, num_instances, num_features)
  print()

def plot_k_for_windows(setname, windows, num_instances, num_features):
  """Plot average accuracy vs k for windowed dataset using multiprocessing."""
  
  window_size = windows[0][0].shape[0] if windows else 0
  k_values = list(range(window_size//100, window_size//10, window_size//100))
...
if __name__ == "__main__":
  ...
  windows, num_inst, num_feat = window_bigset(X, y, window_size=1000)
  plot_k_for_windows('cdc_diabetes_health_indicators', windows, num_inst, num_feat)
