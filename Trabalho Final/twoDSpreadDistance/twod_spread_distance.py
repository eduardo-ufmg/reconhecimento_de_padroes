...
def mean_intra_class_dist(Q0: NDArray[np.float32], Q1: NDArray[np.float32]) -> np.float32 | None:
    """
    ...
    Returns:
        tuple[np.float32, np.float32] | None: A tuple containing the mean distance of distances within the class,
                                    or None if the input is invalid.
    """

    if Q0.ndim != 1 or Q1.ndim != 1:
        return None # Inputs must be 1D arrays
    if Q0.shape[0] != Q1.shape[0]:
        return None # Inputs must have the same length
    
    num_points = Q0.shape[0]
    if num_points < 2:
        # Need at least two points to form a pair and calculate distance/std dev
        return None
...
def mean_inter_class_dist(Q0: NDArray[np.float32], Q1: NDArray[np.float32], y: NDArray[np.int32]) -> np.float32 | None:
    """
    ...
    Returns:
        np.float32: The mean distance between all pairs of points of opposite classes, or None if the input is invalid.
    """

    if not (Q0.ndim == 1 and Q1.ndim == 1 and y.ndim == 1):
        return None # All inputs must be 1D
    if not (Q0.shape[0] == Q1.shape[0] == y.shape[0]):
        return None # All inputs must have the same length
    if Q0.shape[0] == 0: # No points provided
        return None
    ...
    if points_c0.shape[0] == 0 or points_c1.shape[0] == 0:
        # One or both classes are empty.
        # Cannot compute inter-class distances if one class has no points.
        return None
...
def objective_function(Q0: NDArray[np.float32], Q1: NDArray[np.float32], y: NDArray[np.int32]) -> np.float32 | None:
    ...
    Args:
        ...
        y (NDArray[np.int32]): 1D array of class labels (0 or 1).
    Returns:
        np.float32: The computed objective value, or NaN if the spread cannot be computed for either group. Or None for input errors.
    """

    # Basic input validation (structural errors)
    if not (Q0.ndim == 1 and Q1.ndim == 1 and y.ndim == 1):
        return None 
    if not (Q0.shape[0] == Q1.shape[0] == y.shape[0]):
        return None
    ...
    if val_m_inter_cd is None:
        # This occurs if one or both classes are empty.
        # Per docstring: "NaN if the spread cannot be computed for either group."
        # If a class is empty, its intra-spread also cannot be computed.
    ...
    # Calculate the standard deviation of the intra-class distances
