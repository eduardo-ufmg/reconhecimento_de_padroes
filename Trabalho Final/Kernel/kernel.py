...
# Adjusted threshold for float32 determinant checks.
# np.finfo(np.float32).eps is approx 1.19e-7.
# This threshold checks if determinant is practically zero for float32.
COV_DET_THRESHOLD_F32 = np.float32(1e-7) 
...
def kernel(X: NDArray[np.float32], Y: NDArray[np.float32],
    ...
    if abs(h_squared) < np.finfo(np.float32).tiny:
    ...
    scaled_cov_inv: NDArray[np.float32] = cov_inv / h_squared

    h_pow_n_features = np.float32(h) ** n_features
    
    scaled_norm_factor: np.float32
    if norm_factor == np.float32(0.0):
        scaled_norm_factor = np.float32(0.0)
    elif abs(h_pow_n_features) < np.finfo(np.float32).tiny:
        scaled_norm_factor = np.float32(1.0) # Avoid Inf from norm_factor / tiny_h_pow_n
    else:
    ...
    exp_term = np.exp(np.float32(-0.5) * quadratic_form_values) # Ensure -0.5 is also float32
    ...
    if max_pdf_val <= np.finfo(pdf.dtype).tiny:
        pdf_normalized = np.zeros_like(pdf, dtype=np.float32)
    else:
        pdf_normalized = pdf / max_pdf_val

    return pdf_normalized.astype(np.float32) # Final explicit cast


def kernel_fit(X: NDArray[np.float32], type: str='cov') -> tuple[np.float32, NDArray[np.float32]]:
    """
    Fits kernel parameters (optimized for speed and memory with float32).
    All ops performed in float32, accepting higher numerical stability risk.
    """
    ...
    norm_factor_val: np.float32
    cov_inv_val: NDArray[np.float32]
    
    if type == 'cov':
        if n_samples <= 1:
            raise ValueError(f"Cannot compute covariance for type='cov' with {n_samples} samples.")

        # Perform all covariance calculations in float32
        # np.cov might use float64 internally for accumulation, so cast result.
        cov_matrix_f32 = np.cov(X, rowvar=False).astype(np.float32)

        if n_features == 1 and cov_matrix_f32.ndim == 0:
            cov_matrix_f32 = np.array([[cov_matrix_f32.item()]], dtype=np.float32)
        ...
        # Determinant calculation directly on float32 matrix. Result might be float64 scalar by numpy.
        cov_det_f32 = np.float32(np.linalg.det(cov_matrix_f32))

        if abs(cov_det_f32) <= COV_DET_THRESHOLD_F32 or cov_det_f32 < 0: # Check for negative also, det of PSD should be non-negative
            raise ValueError(
                f"Covariance matrix (float32) is singular or ill-conditioned (determinant: {cov_det_f32:.2e})."
            )

        sqrt_cov_det_f32 = np.sqrt(cov_det_f32) # sqrt on float32
        # Ensure intermediate terms in denominator are float32 if possible, or result casted
        pi_f32 = np.float32(np.pi)
        norm_factor_val = np.float32(1.0) / ( ( (np.float32(2.0) * pi_f32) ** (np.float32(n_features) / np.float32(2.0)) ) * sqrt_cov_det_f32 )
        
        # Inverse calculation directly on float32 matrix
        cov_inv_val = np.linalg.inv(cov_matrix_f32).astype(np.float32) # Ensure result is float32

    elif type == 'scale':
        cov_inv_val = np.eye(n_features, dtype=np.float32)
        pi_f32 = np.float32(np.pi)
        norm_factor_val = np.float32(1.0) / ( (np.float32(2.0) * pi_f32) ** (np.float32(n_features) / np.float32(2.0)) )
