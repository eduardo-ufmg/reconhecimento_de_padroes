...
def vector_spread(Qx: NDArray[np.float64]) -> np.float64 | None:
    """
    ...
    Returns:
        np.float64: A score between 0.0 (exclusive, in practice) and 1.0 (inclusive).
    ...
    """
    try:
        numeric_array = np.asarray(Qx, dtype=np.float64)
    except ValueError:
        # Handles cases where conversion to np.float64 array fails (e.g., list of strings)
        return None
    ...
    return np.float64(score) # Ensure standard Python np.float64

def objective_function(Q0: NDArray[np.float64], Q1: NDArray[np.float64], y: NDArray[np.int32]) -> np.float64 | float:
    """
